var hlparr={
nbr:'<div class=mpds>сколько? \'валюта\' = 1</div><div class=pds><b>cn</b>jp = юань в йенах, <b>3</b>ru<b>eu</b> = 3 рубля в евро и т.д.<br> <b>dd</b> = дата, <b>cc</b> = отчистить</div>',ztm:'затем \'валюта\'',chya:'чья валюта?',vch:'в чью пересчитать?'},btn=document.getElementsByClassName('num'),cnt=document.getElementsByClassName('cnt'),tbl=document.getElementById('tbl'),v=document.getElementById('v'),clr=document.getElementById('clr'),wnd=document.getElementById('wnd'),hlp=document.getElementById('hlp'),dat=document.getElementById('dat'),seg=document.getElementById('seg'),curs,cursV,anch,epi=keyadd=data=chsl=msc=gd=hshv=!1,sklk=curs_1=curs_1V=Vtxt=1,morenum=0,msc=['января','февраля','марта','апреля','мая','июня','июля','августа','сентября','октября','ноября','декабря'],bbarr={
'md':MDL,'by':BYN,'ua':UAH,'ch':CHF,'uk':GBP,'tr':TRY,'us':USD,'ru':RUB,'eu':EUR,'in':INR,'cn':CNY,'jp':JPY,'dd':dat,'cc':clr,'kz':KZT,'am':AMD
},digarr={
48:'0',49:1,50:2,51:3,52:4,53:5,54:6,55:7,56:8,57:9,190:'.',188:'.',96:'0',97:1,98:2,99:3,100:4,101:5,102:6,103:7,104:8,105:9,110:'.'
},keyarr={
65:'a',66:'b',67:'c',68:'d',69:'e',70:'f',71:'g',72:'h',73:'i',74:'j',75:'k',76:'l',77:'m',78:'n',79:'o',80:'p',81:'q',82:'r',83:'s',84:'t',85:'u',86:'v',87:'w',88:'x',89:'y',90:'z',8:''}
...
*/
	var xhr=new XMLHttpRequest();
	if(V==1||dt) xhr.open('GET','13-02-2018.json',!1);
	else  xhr.open('GET','16-02-2018.json',!1);
	xhr.send(null);
	if(xhr.status!=200)hlp.innerHTML='ошибочка: '+xhr.status+': '+xhr.statusText;
	else return JSON.parse(xhr.responseText);
}
